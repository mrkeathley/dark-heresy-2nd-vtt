...
export async function psychicPowerRoll(rollData) {
...
async function _sendToChat(rollData) {
    ...
    const html = await renderTemplate('systems/dark-heresy-2nd/templates/chat/psychic-power-roll-chat.hbs', rollData);
    let chatData = {
        user: game.user.id,
